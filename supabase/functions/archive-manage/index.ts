...
//
// A pasta-raiz "Acervo Público - OneMed" fica FORA da pasta da biblioteca de
// cursos — a sincronização varre a biblioteca e transformaria o acervo em
// "curso". O id fica salvo em drive_config.acervo_folder_id.
// ─────────────────────────────────────────────────────────────────────────────
...
async function getDriveToken(supabase: ReturnType<typeof createClient>): Promise<{ token: string; config: any }> {
  const { data: config, error } = await supabase.from('drive_config').select('*').single()
  if (error || !config?.connected) throw new Error('Google Drive não configurado')
  ...
  if (!expiry || expiry.getTime() < Date.now() + 5 * 60 * 1000) {
    token = await refreshAccessToken(config.refresh_token, Deno.env.get('GOOGLE_CLIENT_SECRET')!)
...
async function ensureAcervoRoot(supabase: ReturnType<typeof createClient>, token: string, config: any): Promise<string> {
  if (config.acervo_folder_id) return config.acervo_folder_id as string
  ...
  await supabase.from('drive_config').update({ acervo_folder_id: data.id }).eq('id', config.id)
...
serve(async (req) => {
  ...
  try {
    ...
    // ── INIT ─────────────────────────────────────────────────────────────────
    if (action === 'init') {
      const { token, config } = await getDriveToken(supabase)
      ...
      if (itemId) {
        ...
        const rootId = await ensureAcervoRoot(supabase, token, config)
    ...
    if (action === 'finalize') {
      ...
      const { data: item } = await supabase.from('archive_items')
        .select('id, user_id').eq('id', itemId).maybeSingle()
      if (!item || item.user_id !== user.id) return json({ error: 'Item não encontrado.' }, 404)

      const { token } = await getDriveToken(supabase)
