...
serve(async (req) => {
  ...
  try {
    ...
    // Guard de tamanho do corpo (o cronograma é só texto — 4MB é folga enorme).
    ...
    // Rate-limit ATÔMICO via RPC + FAIL-CLOSED (a IA é paga por chamada).
    try {
      const { data: h } = await supabase.rpc('rate_limit_hit', {
        _identifier: user.id, _action: 'study_plan', _limit: LIMITE_DIARIO, _window_seconds: 24 * 3600,
      })
      const rl = Array.isArray(h) ? h[0] : h
      if (rl && rl.allowed === false) {
        // Janela de 24h desde a PRIMEIRA geração.
